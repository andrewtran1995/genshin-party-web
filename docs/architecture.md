...
- **`src/lib/genshin/characters.ts`** and **`src/lib/genshin/bosses.ts`** each load only their own JSON and expose their own pure functions — `getChars({ element, rarity, includeTraveler, exclude })`/`getRandomChar(...)` and `getBosses({ weekly })`/`getRandomBoss(...)`/`getRandomBosses(...)` respectively — so a route that only reads one dataset doesn't bundle the other. `order.ts` holds the party-order permutation helpers, and `sample.ts` the shared random-pick helper both use. `index.ts` re-exports all four as a barrel for convenience; it has no data-loading code of its own. This layer is client-safe, so all randomization runs in the browser.
